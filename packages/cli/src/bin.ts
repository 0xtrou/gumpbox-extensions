...
function printHelp(): void {
  console.log(`gumpbox CLI v${VERSION}
...
async function proxy(): Promise<void> {
  const clientName = process.env.GUMPBOX_CLIENT_NAME ?? "gumpbox-cli";
...
async function status(): Promise<void> {
  ...
  try {
    ...
    const info = await client.initialize({ name: "gumpbox-cli", version: VERSION });
...
async function seedSkills(): Promise<void> {
  ...
  const client = new MCPClient(cfg);
  await client.initialize({ name: "gumpbox-cli", version: VERSION });
...
async function install(arg: ClientName): Promise<void> {
  ...
  console.log(`Done. Next: install the CLI globally with 'npm install -g @gumpbox/cli'.`);
}

function proxyCommand(): string {
  return process.platform === "win32" ? "gumpbox.cmd" : "gumpbox";
}
...
function writeGeminiConfig(home: string): void {
  ...
  const entry = { command: proxyCommand(), args: proxyArgs() };
  if (existsSync(path)) {
